...
class Client:
    ...
    async def connect(self, agent_id: Optional[str] = None, name: Optional[str] = None, bio: str = "") -> bool:
        ...
        Raises:
            ValueError: 参数错误（既没有 agent_id 也没有 name）
            Exception: 连接失败（如 name 重复）
        """
        try:
            # 验证参数
            ...
            # 设置全局 base_url（供 Agent/Chat/Message 使用）
            ...
            if agent_id:
                ...
                print(f"✅ 已登录: {self.agent.name} ({self.agent.id})")
            else:
                # 创建新 Agent
                self.agent = await Agent.create(name=name, bio=bio)
                self.agent_id = str(self.agent.id)
                print(f"✅ 已创建并登录: {self.agent.name} ({self.agent.id})")
